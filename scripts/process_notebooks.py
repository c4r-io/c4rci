"""Process notebook units for Community for Rigor
...
GITHUB_RAW_URL = (
    f"https://raw.githubusercontent.com/c4r-io/{REPO}/{MAIN_BRANCH}"
)
GITHUB_TREE_URL = (
    f"https://github.com/c4r-io/{REPO}/tree/{MAIN_BRANCH}"
)
...
def main(arglist):
    ...
    for nb_path, nb in notebooks.items():
        ...
        # Write the original notebook back to disk, clearing outputs only for units
        print(f"Writing complete notebook to {nb_path}")
        with open(nb_path, "w") as f:
            nb_clean = clean_notebook(nb, clear_outputs=nb_path.startswith("units"))
            nbformat.write(nb_clean, f)

        # if the notebook is not in units, skip the creation/update of the student, static, solutions directories
        if not nb_path.startswith("units"):
          continue
...
def extract_solutions(nb, nb_dir, nb_name):
    ...
    nb = deepcopy(nb)
    _, unit_dir = os.path.split(nb_dir)
    ...
    for i, cell in enumerate(nb_cells):
        ...
        if has_solution(cell):
            ...
            py_url = f"{GITHUB_TREE_URL}/units/{unit_dir}/{py_fname}"
            ...
            if cell_images:
                ...
                for f, img in cell_images.items():
                    ...
                    url = f"{GITHUB_RAW_URL}/units/{unit_dir}/{f}"
...
def redirect_colab_badge_to_main_branch(cell):
    ...
    p = re.compile(r"^(.+/c4r-io/" + REPO + r"/blob/)[\w-]+(/.+$)")
...
def redirect_colab_badge_to_student_version(cell):
    ...
    p = re.compile(r"(^.+blob/" + MAIN_BRANCH + r"/units/C\dU\d\w+)/(\w+\.ipynb.+)")
    cell_text = p.sub(r"\1/student/\2", cell_text)
    # redirect the kaggle badge
    p = re.compile(r"(^.+/units/C\dU\d\w+)/(\w+\.ipynb.+)")
...
def test_redirect_colab_badge_to_student_version():
    ...
    expected = (
        "\"https://colab.research.google.com/github/c4r-io/"
        "default-repo/blob/main/units/C1U1_ModelTypes/student/"
        "C1U1_MiniUnit1.ipynb\""
    )
